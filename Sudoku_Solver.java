...
public class Sudoku_Solver {
            ...
            for (int j = 0; j < 9; j++) {
                if (board[i][j][0] == 0) {
                    ...
                    if (board[i][j][1] == board[i][j][2] && board[i][j][2] == board[i][j][3] && board[i][j][3] == board[i][j][4] && board[i][j][4] == board[i][j][5] && board[i][j][5] == board[i][j][6] && board[i][j][6] == board[i][j][7] && board[i][j][7] == board[i][j][8] && board[i][j][8] == board[i][j][9]) {
                        System.out.printf("%d ", board[i][j][0]);
                    } else {
                        System.out.printf("%s%d%s ", blue, board[i][j][0], reset);
                    }
                }
            }
            System.out.println();
    ...
    public static void SolveBoard() {
        ...
        while (true) {
                    ...
                    if (protection >= 75) {
                        for (int j = 1; j < 10; j++) {
                            ...
                            if (counter == 1) {
                                for (int k : Square[square]) {
                                    if (board[order[k][0]][order[k][1]][j] != 0) {
                                        board[order[k][0]][order[k][1]][0] = j;
                                        for (int l = 1; l < 10; l++) {
                                            if (board[order[k][0]][order[k][1]][l] != board[order[k][0]][order[k][1]][0]) {
            ...
            DrawBoard();
//            System.out.println(Arrays.toString(board[2][0]));
//            scanner.nextLine();
            if (protection >= 100) {
                while (true) {
                    int[] in;
    ...
    public static void UseArgs(String[] args) {
        ...
        for (int i = 0; i < 81; i++) {
            x = order[i][0];
            y = order[i][1];
            board[x][y][0] = Integer.parseInt(args[i]);
        }
    }
}
